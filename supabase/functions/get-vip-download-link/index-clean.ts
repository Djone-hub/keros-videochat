// Файл: supabase/functions/get-vip-download-link/index.ts
// Эта функция генерирует безопасную, временную ссылку для скачивания VIP-модов.
...
// Генерация уникального токена для внешних ссылок
function generateDownloadToken(userId: string, modId: string): string {
  const timestamp = Date.now()
  const random = Math.random().toString(36).substring(2, 15)
  return btoa(`${userId}-${modId}-${timestamp}-${random}`)
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (mod.file_path) {
      ...
      // 🔐 Для внешних ссылок (ShareMods и т.д.) - создаем временный токен
      isExternalLink = true
      console.log('🔗 [DOWNLOAD] Используем внешнюю ссылку:', mod.download_url)
      const downloadToken = generateDownloadToken(user.id, modId)
      
      // Сохраняем токен в базу данных с временем истечения (5 минут)
      ...
      if (tokenError) {
        ...
        // Не прерываем, просто используем прямую ссылку
        downloadUrl = mod.download_url
      } else {
        // Возвращаем ссылку на Edge Function которая проверит токен
        downloadUrl = `${Deno.env.get('SUPABASE_URL')}/functions/v1/download-external-file?token=${downloadToken}`
        console.log('🔐 [DOWNLOAD] Сгенерирован временный токен для внешней ссылки')
      }
    } else {
      // ❌ Нет ни file_path ни download_url
    ...
    try {
      await adminSupabase
        ...
        .insert({
          ...
          is_vip_mod: isVipMod,
          download_url: downloadUrl.substring(0, 200), // Сохраняем только начало URL
          created_at: new Date().toISOString()
        })
      ...
      // Не прерываем скачивание из-за ошибки логирования
    }
    ...
    return new Response(JSON.stringify({
      ...
      expiresInSeconds: isExternalLink ? 300 : 60, // 5 минут для внешних, 60 секунд для Storage
